...
include_graphics("figures/rstudio_managing.png")
```
...
## {data-background="figures/rmarkdown_file.png"}
...
- **Open R Studio and create an R project**: File > New Project... > Select Version Control > Select Git > Enter the repository URL (the URL you just copied: <https://github.com/USERNAME/REPOSITORY-NAME>), choose a project directory name and where it should be saved. Click "Create Project". 
    - <span style="color: green; font-size: 1;">(Terminal alternative: 'git clone https://github.com/YOUR-USERNAME/YOUR-REPOSITORY.git'. Then 'cd YOUR-REPOSITORY')</span> 
- **Make changes to README file, commit, and push**
  ...
  + Make changes to your README ("This is my first GitHub repository") 
    - <span style="color: green; font-size: 1;">(Terminal alternative: 'echo "YOUR-TEXT" >> README.md')</span>
  ...
  + **Push to GitHub**: Click on "Push" 
    - <span style="color: green; font-size: 1; ">( Terminal alternative: "git add -A'. Then 'git commit -m "COMMIT-MESSAGE"'. Then 'git push')</span>
...
- **Make changes to your Markdown file**: Enter a first section title (`# Random number generation`), a text ("Here, we generate 10 random numbers between 0 and 10"), and an r code chunk (`'''{r} numbers <- sample(0:10, 10, replace = FALSE)'''`). 
...
- renv only keeps track of packages that are actually used in one or more of your scripts. Therefore, create a new code chunk in your RMarkdown file and paste:
